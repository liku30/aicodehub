    ...
    <div class="container-narrow" style="padding-top:100px;padding-bottom:40px;">
      <div class="card animate-in">
        ...
        <el-form :model="form" label-position="top" style="max-width:480px;">
          ...
          <el-form-item><el-button type="primary" :loading="loading" @click="handleSave" style="border-radius:9999px;">保存修改</el-button></el-form-item>
        </el-form>
      </div>
    </div>
  </div>
...
onMounted(async () => { try { const r = await getProfile(); form.nickname = r.data.nickname || ''; form.bio = r.data.bio || ''; form.githubUrl = r.data.githubUrl || '' } catch {} })
async function handleSave() { loading.value = true; try { await updateProfile(form); ElMessage.success('已保存') } catch {} finally { loading.value = false } }
</script>
